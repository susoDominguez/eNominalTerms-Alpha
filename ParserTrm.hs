...
--parsePrm :: Parser Prm
--parsePrm = (char '-' >> return []) <|> fmap reverse (many1 parseTrans) <?> "permutation"
...
parseaCtx = fmap S.fromList (between (char '{' >> spaces) (spaces >> char '}') (sepBy p (char ','))) <?> "freshness context"
    where p = do a <- (spaces >> parseAtm)
                 ...
                 x <- parseVar
                 spaces
                 return (a, x) <?> "Assumption"
                 
parseTrm :: Parser Trm
parseTrm = parseAbsTrm <|> parseAppTrm <|> (fmap AtmTrm parseAtm) <|> (try parseTplTrm <|> parseVarTrm) <?> "term"
...
parseRules = do rls <- sepBy parseRule (string "\n")
                spaces
                eof
                return rls
